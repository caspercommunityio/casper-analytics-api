...
class ProcessHistory extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'api:ProcessHistory';
     ...
     * @var string
     */
    protected $description = 'Process the history of each blocks to get data (delegation, undelegation, etc)';
    ...
    public function getBlockDetail($blockHash, $counter, $max)
    {
        ...
        $deploys = isset($dataBlock['result']['block']['body']['deploy_hashes']) ? $dataBlock['result']['block']['body']['deploy_hashes'] : array();
        $transfers = isset($dataBlock['result']['block']['body']['transfer_hashes']) ? $dataBlock['result']['block']['body']['transfer_hashes'] : array();
        ...
        if (!BlocksProcessed::where(array('blockHash'=>$blockHash))->exists()) {
            ...
            BlocksProcessed::where(array('blockHash'=>$blockHash))->update(array("switchBlock" => (isset($dataBlock['result']['block']['header']['era_end']) ? true:false), "processed" => (isset($dataBlock['result']['block']['header']['era_end']) ? false : true),"deploys" => count($deploys), "transfers" => count($transfers), "eraId" => (isset($dataBlock['result']['block']['header']['era_id']) ?  $dataBlock['result']['block']['header']['era_id'] : -1), "blockHeight" => (isset($dataBlock['result']['block']['header']['height']) ? $dataBlock['result']['block']['header']['height'] : -1),"deploymentDate" => substr($dataBlock['result']['block']['header']['timestamp'], 0, -6)));
        }
    ...
    public function handle()
    {
        ...
        $blockToProceed=$lastProcessedBlock->blockHash;
        $counter=0;
        $result = $this->getBlockDetail($blockToProceed, $counter, 5000);
    }
}
